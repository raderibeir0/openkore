...
sub calc_checksum {
    my @crc8_table = (
        
    );
    
    my ($counter, $data) = @_;
    my @data_bytes;
    
    # Verifica se é string hexadecimal (apenas 0-9, A-F, a-f)
    if ($data =~ /^[0-9A-Fa-f]+$/ && length($data) % 2 == 0) {
        # Converte string hex para bytes
        @data_bytes = map { hex($_) } ($data =~ /../g);
    } else {
        # Converte string normal para array de bytes ASCII
        @data_bytes = map { ord($_) } split(//, $data);
    }
    
    # Calcula checksum inicial com o counter
    my $v10 = $crc8_table[
        (($counter >> 24) & 0xFF) ^ $crc8_table[
            (($counter >> 16) & 0xFF) ^ $crc8_table[
                (($counter >> 8) & 0xFF) ^ $crc8_table[
                    $counter & 0xFF
                ]
            ]
        ]
